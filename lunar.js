...
var Lunar = (function() {
    // 农历数据表 1900-2100
    // 每年用一个16进制数表示：
    // 前4位：闰月的天数（0=29天，1=30天），如果没有闰月则为0
    // 中间4位：闰月月份（0=无闰月）
    // 后12位：每月大小月（1=30天，0=29天）
    var lunarInfo = [
    ...
    function solar2lunar(sy, sm, sd) {
        ...
        var objDate = new Date(sy, sm - 1, sd);
        ...
        return {
            ...
            monthCN: (isLeap ? '闰' : '') + lunarMonthName[month - 1] + '月',
            dayCN: lunarDayName[day - 1],
            fullCN: tianGan[ganIdx] + diZhi[zhiIdx] + '年 ' + (isLeap ? '闰' : '') + lunarMonthName[month - 1] + '月' + lunarDayName[day - 1]
        };
    }
